...
Every ratio here is zero when its denominator is zero, which only occurs on a
candle with no high-to-low range. That convention is applied uniformly so a
flat candle produces a defined value rather than a missing one.
"""
...
def body_ratio(frame: pd.DataFrame) -> pd.Series:
    ...
    return safe_divide(frame["close"] - frame["open"], frame["high"] - frame["low"])
...
def close_location(frame: pd.DataFrame) -> pd.Series:
    ...
    return safe_divide(frame["close"] - frame["low"], frame["high"] - frame["low"])
...
def upper_wick_ratio(frame: pd.DataFrame) -> pd.Series:
    ...
    return safe_divide(frame["high"] - body_top, frame["high"] - frame["low"])
...
def lower_wick_ratio(frame: pd.DataFrame) -> pd.Series:
    ...
    return safe_divide(body_bottom - frame["low"], frame["high"] - frame["low"])
...
def register_price_features(registry: FeatureRegistry) -> None:
    ...
    definitions = (
        FeatureDefinition(
            ...
            description="Body (close minus open) as a fraction of the high-low range.",
            lookback=1,
            compute=body_ratio,
        ),
        FeatureDefinition(
        ...
        FeatureDefinition(
            ...
            group=FeatureGroup.PRICE,
            description="Position of the close within the candle's high-low range.",
            lookback=1,
        ...
        FeatureDefinition(
            ...
            group=FeatureGroup.PRICE,
            description="Upper wick as a fraction of the candle's high-low range.",
            lookback=1,
        ...
        FeatureDefinition(
            ...
            group=FeatureGroup.PRICE,
            description="Lower wick as a fraction of the candle's high-low range.",
            lookback=1,
